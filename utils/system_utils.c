...
int eraseFolderExceParamFile(const char *folder, const char* file_name, const char *model_num)
{
    ...
    while((dir = readdir(folder_fd)) != NULL) {
        if (dir->d_type == DT_DIR || (strcasecmp(dir->d_name, file_name) == 0) || (strcasecmp(dir->d_name, pdri_file_name)==0) || strcasecmp(dir->d_name, file_hdr) == 0 ||
        strcasecmp(dir->d_name, pdri_hdr) == 0) {
            continue;
        } else {
